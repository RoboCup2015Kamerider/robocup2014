...
{  
	cout << "f_pos = " << f_posX << "\t" << f_posZ << endl;
	cout << "f_dir = " << f_dirX << "\t" << f_dirZ << endl;
	cout << "d_pos = " << d_posX << "\t" << d_posZ << endl;
	...
	else if(m_state == 30)
	{
		...
		if(DetectCococup(cpt, 1))
		{
			cordinate cor;
			int dis = GETPOINTDIS(cpt, cor, 1);  
			cout << "\t###################### dis = " << dis << "\t" << cpt.x << endl;
			if(cpt.x < 150)
		        {
		        	DIRECTIONCORRECTION((cpt.x - 150) / 10);
		        	cout << "cpt < 150" << endl;
		        }
		        else if(cpt.x > 170)
		        {
		                DIRECTIONCORRECTION((cpt.x - 170) / 10);
		                cout << "cpt > 170" << (cpt.x - 170) / 10 << endl;
		        }
			else if(dis < 20)
		        {
		        ...
		        else if(dis > 25)
		        {
		                ...
		                cout << "position correction " << (dis - 25) / 3 << endl;
		        }
		        else
		        {
		         	cout << "POSITION CORRECT" << endl;
		         	m_state = 40;
		        }
		}
	}
...
{
	ViewImage *dis_img = m_view->distanceSensor1D(0.0, 1023.0, camID, DEPTHBIT_8, IMAGE_320X240);  
